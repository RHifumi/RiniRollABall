...
-`rb.AddForce(vectorMovimiento * Velocidad);` -> Para aplicar fuerzas a la esfera en base a los inputs de `WASD`. 
-`other.gameObject.CompareTag("Coleccionable") == true){} ` ->	Una vez detectada la colisión con los colliders, observar si se trata de un coleccionable con su tag. 
-`other.gameObject.SetActive(false);` -> Para deshabilitar los coleccionables desde la esfera una vez se detecta la colisión y dejar los coleccionables con menos responsabilidad. 
-`Interfaz UI Canvas` -> Texto en pantalla para señalizar cantidad de coleccionables obtenidos y mostrar texto de Victoria. 
---
## Scripts Principales
